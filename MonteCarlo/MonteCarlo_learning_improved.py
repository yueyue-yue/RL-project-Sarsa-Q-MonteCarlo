...
actions:
    ...
    1 ----> down
    2 ----> right
    This improved policy is found in a bug. When we let the agent make one more step,
    it will find the goal very quickly.
    Maybe in this way, the agent can get more information about the place surrounded the holes or goals.
'''
...
def first_visit_data_process(location_info, action_info, reward_info):
    ...
    for ii in range(len(location_info) - 1, 0, -1):
        # del the same element according to the state
        for j in range(0, ii):
            # print(f'a[i]:{location_info[i]},a[j]:{location_info[j]}')
            if location_info[ii] == location_info[j]:
                try:
                    del location_info[ii]
                    del action_info[ii]
                    del reward_info[ii-1]
                    break
                except IndexError:
                    print('')
            # There is always IndexError after the program has been finished.
            # The try function can let the program neglect this meaningless error.
            # It does't have any effect on the process of MonteCarlo learning but wasting time.
    return location_info, action_info, reward_info
...
for i in range(10):
    a, b, c = m_test(10)
    running_time_M_.append(a)
    episode_M_.append(b)
    plot_sum_reward_M_.append(c)
'''
...
if __name__ == "__main__":
    ...
    T1 = time.perf_counter()
    # update(judge_number=2, judge_method='sum of episodes', delay_time=0.00)
    update(judge_number=20, judge_method='repeated steps', delay_time=0.00)
    T2 = time.perf_counter()
    ...
    np.set_printoptions(threshold=len(MonteCarlo_brain_.table_result()))
...
'''
running_time_, episode_, plot_sum_reward_ = 0, 0, 0
    running_time, episode, plot_sum_reward = m_test(20)
    running_time_ += running_time
    episode_ += episode
    # plot_sum_reward_ += plot_sum_reward_
print('运行时间:', running_time_/10)
print('平均episode:', episode_/10)
'''
