...
class Qlearning:
    ...
    def table_calculate_(self, state_current_, action_current_, state_next, reward_current,
                         step_info, last_step):

        current_value = self.locate_table_value(state_current_, action_current_)

        # This part is special for Q brain.
        next_value = max([
            self.locate_table_value(state_next, 0), self.locate_table_value(state_next, 1),
            self.locate_table_value(state_next, 2), self. locate_table_value(state_next, 3)])
        # If this is the last step, we set the next state value to be 0 according to the algorithm.
        if last_step:
            next_value = 0
